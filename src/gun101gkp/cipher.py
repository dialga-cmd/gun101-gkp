...
def encrypt(plaintext: bytes, key: bytes, associated_data: Optional[bytes] = None) -> tuple[bytes, bytes, bytes]:
    ...
    assert len(key) == DEK_LEN  # nosec B101
    nonce = os.urandom(AES_NONCE_LEN)
    # Intentional invariant; exercised by the Hypothesis dynamic-analysis suite.
    assert len(nonce) == AES_NONCE_LEN  # nosec B101
    aesgcm = AESGCM(key)
...
def decrypt(nonce: bytes, ciphertext: bytes, tag: bytes, key: bytes, associated_data: Optional[bytes] = None) -> bytes:
    ...
    # Intentional invariant; exercised by the Hypothesis dynamic-analysis suite.
    assert len(ciphertext_with_tag) == len(ciphertext) + 16  # nosec B101
    aesgcm = AESGCM(key)
    try:
